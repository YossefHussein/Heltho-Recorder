import 'package:flutter/material.dart';

/*
   p from primary.
   s from secondary
 */

/// these are main colors of my application
Color pColor = Color(0xFF29153D);
Color sColor = Color(0xFF301155);

/// colors of button and item
Color pColorText = Color(0xFFFFFFFF);
Color pColorIcon = Color(0xFFFFFFFF);
Color pColorSliderInactive = Color(0xFFBDBDBD);
Color pColorResultButton = Color(0xFFF44336);
Color pColorButton = Color(0xFF462777);

/// and these are some values to I use in this app and you can use
/// these values like size padding and other values to make your application

const double pPadding = 20;
const double pBorderSize = 20;
const double pSizeBox = 15;
const double pIconSize = 50;

/// font size
const double pLargeFontSize = 30.0;
const double pMediumFontSize = 20;
const double pSmallFontSize = 15;

const kTextColor = Color(0xFF535353);
const kTextLightColor = Color(0xFFACACAC);
